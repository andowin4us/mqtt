...
async function processMessage(data) {
    console.log('New message device', data?.device_id, 'for log type', data?.log_type, '=>', data.log_desc, 'with log line count', data?.log_line_count);
...
async function handleOtherLogs(data, result, getFlagData) {
    ...
    if (data.log_type === 'DOOR' && data.log_desc === 'OPENED') {
        ...
        if (!checkMaintainence) {
            await mongoInsert({ status: 'InActive', "mqttStatusDetails.mqttRelayState": true }, { deviceId: data.device_id }, 'MQTTDevice', 'update');
            const MQTT_URL = `mqtt://${result.mqttIP}:${result.mqttPort}`;
